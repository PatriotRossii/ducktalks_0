...
template <typename T, std::size_t N> class chunk {
  ...
  public:
    ...
    auto operator[](std::size_t idx) const -> const T & {
        return (type_ == ChunkType::reversed ? data[N - idx - 1] : data[idx]);
    }
    ...
    auto begin() const -> const T * {
        return (type_ == ChunkType::reversed ? data.rbegin() : data.begin());
    }
    auto end() const -> const T * {
        return (type_ == ChunkType::reversed ? data.rend() : data.end());
    }
